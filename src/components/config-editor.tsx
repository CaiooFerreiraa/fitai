...
export function ConfigEditor({ programId }: { programId?: string }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    try {
      await saveWorkoutPlanAction({ dayOfWeek, exercises }, programId)
  ...
  const handleMove = async () => {
    if (!targetMoveDay) return
    setLoadingMove(true)
    try {
      await moveWorkoutPlanAction(dayOfWeek, targetMoveDay as DayOfWeek, programId)
      toast.success("TREINO REDIRECIONADO", {
        description: `TREINO MOVIDO PARA ${DAY_LABELS_PT[targetMoveDay as DayOfWeek]}.`
      })
      setDayOfWeek(targetMoveDay as DayOfWeek)
      setTargetMoveDay("")
    } catch {
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-10 pt-6 sm:pt-10 md:pt-12 space-y-6 md:space-y-10">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 md:gap-10 items-start">
          ...
          <div className="xl:col-span-3 space-y-4 md:space-y-6">
            ...
            {loadingDay ? (
              ...
              <div className="bg-[#121214] border-4 border-dashed border-[#1c1c1f] rounded-3xl p-10 md:p-14 text-center group hover:border-[#ff0033]/20 transition-all relative overflow-hidden">
                <div className="w-24 h-24 mx-auto mb-6 rounded-2xl bg-[#0a0a0b] flex items-center justify-center border-4 border-[#1c1c1f] shadow-[6px_6px_0_0_#000]">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            {/* Move Day Section */}
            {exercises.length > 0 && !loadingDay && (
              <div className="bg-[#121214] border-4 border-black p-4 sm:p-5 rounded-3xl shadow-[4px_4px_0_0_#000] relative overflow-hidden group hover:border-[#ff0033]/20 transition-all">
                <Label className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.4em] text-neutral-600 block mb-3 italic">
                  MOVER TREINO PARA OUTRO DIA:
                </Label>
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                  <Select value={targetMoveDay} onValueChange={(val: DayOfWeek) => setTargetMoveDay(val)}>
                    <SelectTrigger className="flex-1 bg-black/60 border-2 border-[#1c1c1f] focus:border-[#ff0033] focus:ring-0 text-sm font-black uppercase tracking-tight rounded-xl italic px-4 shadow-[2px_2px_0_0_#000] h-12">
                      <SelectValue placeholder="SELECIONE..." className="text-neutral-500" />
                    </SelectTrigger>
                    <SelectContent className="bg-[#1c1c1f] border-4 border-black text-white rounded-xl">
                      {DAYS_OF_WEEK.filter(d => d !== dayOfWeek).map((day) => (
                        <SelectItem key={day} value={day} className="uppercase tracking-tighter font-black py-2.5 text-sm italic focus:bg-[#ff0033]">
                          {DAY_LABELS_PT[day]}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <Button 
                    onClick={handleMove}
                    disabled={!targetMoveDay || loadingMove}
                    className="h-12 bg-neutral-800 hover:bg-[#ff0033] text-white rounded-xl font-black text-sm uppercase italic shadow-[3px_3px_0_0_#000] border-2 border-black transition-all px-4 shrink-0 sm:w-auto w-full"
                  >
                    {loadingMove ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : <ArrowRightLeft className="w-5 h-5" strokeWidth={3} />}
                  </Button>
                </div>
              </div>
            )}
